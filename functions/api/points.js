...
// }
// GET ?user=<l>  -> { login, name, logins, lv, lvProgress, lvDaysLeft, loginRank, lvMax }
//
// 登录榜规则（MVP，纯计算、无需写入）：
//   累计「活跃天数」（每次 /api/me 且当天未记过时 +1），再按等级公式换算 Lv。
//   注：原「帖子榜」随小蓝页帖子系统下线一并移除（2026-09-23）。
...
export async function onRequestGet(context) {
  ...
  const [idx, loginMap] = await Promise.all([
  ...
  if (user) {
    ...
    return json(Object.assign(
      ...
      info,
      { lvBaseDays: LV_BASE, lvStepDays: LV_STEP }
    ));
  }

  const withRank = (rows) => rows.slice(0, 50).map((r, i) => Object.assign({ rank: i + 1 }, r));
  return json({
    loginBoard: withRank(loginRows),
    lvMax: LV_MAX,
    lvBaseDays: LV_BASE,
    lvStepDays: LV_STEP,
    perLevel: LV_BASE,
